...
def load_scheduler_config() -> dict | None:
    ...
    try:
        return json.loads(_SCHEDULER_CONFIG.read_text())
    except Exception:
        return None


def save_scheduler_config(backend: str, python: str) -> None:
...
def detect_scheduler_backends() -> dict:
    ...
    # launchd (macOS)
    r = subprocess.run(["launchctl", "version"], capture_output=True, text=True)
    backends["launchd"] = {
    ...
    # systemd user (Linux)
    r = subprocess.run(
        ["systemctl", "--user", "--version"], capture_output=True, text=True
    )
    backends["systemd"] = {
        "available": r.returncode == 0,
        "notes": r.stdout.strip().split("\n")[0]
    ...
    r = subprocess.run(["schtasks", "/?"], capture_output=True)
    backends["windows"] = {
        "available": r.returncode == 0,
...
def install_watchdog(backend: str, python: str | None = None) -> bool:
    ...
    Returns True if newly installed, False if already configured identically.
    """
    python = python or sys.executable
    config = load_scheduler_config()
    if config and config.get("backend") == backend and config.get("python") == python:
        return False
