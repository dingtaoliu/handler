...
def _oauth_required_message(auth_url: str, run_ctx=None) -> str:
    ...
    return (
        ...
        f"3. Call: complete_google_auth(service='gmail', code_or_url='<paste URL or code here>')\n\n"
        f"Alternatively, on the server: handler auth gmail --console --user {user_id}"
    )
    ...
    if not creds or not creds.valid:
        ...
        if not creds or not creds.valid:
            ...
            if _is_headless():
                ...
                store_pending_flow("gmail", user_id, flow)
