...
from ..memory import _validate_filename
...
def create_admin_router(
    ...
    async def upload(files: list[UploadFile] = File(...)):
        ...
        for f in files:
            ...
            logger.info(f"upload: {f.filename} ({len(content)} bytes) -> {dest.resolve()}")
    ...
    async def recover(token: str = ""):
        ...
        if not expected:
            return {"error": "recovery endpoint disabled (HANDLER_RECOVER_TOKEN not set)"}
        if token != expected:
