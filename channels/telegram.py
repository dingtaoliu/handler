...
class TelegramChannel(Channel):
    ...
    async def _on_photo(self, update, context) -> None:
        """Handle photos: download, save, and send path + caption to agent."""
        ...
        local_path = _UPLOAD_DIR / f"tg_{update.message.chat_id}_{update.message.message_id}{ext}"
        await file.download_to_drive(str(local_path))
        logger.info(f"telegram photo saved: {local_path}")

        caption = update.message.caption or ""
        content = f"[Photo saved to {local_path.resolve()}]\n{caption}".strip()
    ...
    async def _process(self, update, content: str) -> None:
        ...
        typing_task = asyncio.create_task(
            self._typing_loop(update.message.chat)
        )
